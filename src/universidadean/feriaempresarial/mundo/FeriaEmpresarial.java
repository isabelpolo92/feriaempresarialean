...
public class FeriaEmpresarial {
    ...
    private HashMap<String, Integer> zoneParticipants;
    ...
    public String metodo1() {
    	List<Empresa> empresas = this.darEmpresas();
    	
    	Puesto[]  sits = this.darPuestos();
    	
    	this.response = 0;
    	this.zoneParticipants = new HashMap<String, Integer>();
    	
    	for(int x=0; x<sits.length; x++) {
    		this.currentCompany = sits[x].darNombreEmpresa();
    		this.currentZone = sits[x].darZona();
    		if(!sits[x].darNombreEmpresa().isEmpty()) {
    			empresas.forEach( emp -> {
    				if(emp.darNombre().equals(this.currentCompany)) {
    					int currentCompanyVisitors = zoneParticipants.get(this.currentZone) != null ? zoneParticipants.get(this.currentZone) : 0;
    					zoneParticipants.put(this.currentZone, currentCompanyVisitors + emp.darNumeroPersonasAsistentes());    					
    				}
    	    	});
    		}
    	...
    	int max = Collections.max(zoneParticipants.values());
    	
    	List<String> list =  zoneParticipants.entrySet().stream()
        .filter(entry -> entry.getValue() == max)
        .map(entry -> entry.getKey())
        .collect(Collectors.toList());
    	
        return "La zona más concurrida es "+list.get(0)+" y tiene "+max+" participantes";
    }
...
List<Empresa> empresas = this.darEmpresas();
    	...
    	for(int x=0; x<sits.length; x++) {
    		this.currentCompany = sits[x].darNombreEmpresa();
    		this.currentZone = sits[x].darZona();
    		maxExpositoresFeria += sits[x].darNumMaximoPersonas();
    		
    		if(!sits[x].darNombreEmpresa().isEmpty()) {
    			empresas.forEach( emp -> {
    				if(emp.darNombre().equals(this.currentCompany)) {
    					totalParticipants += emp.darNumeroPersonasAsistentes();
    				}
    	    	});
    		}    		
    	}
    	float result = this.totalParticipants*100/maxExpositoresFeria;
    	
    	String response="El punto de equilibrio se ha cumplido";
    	
    	if(result<60) {
    		response = "El punto de equilibrio no se ha cumplido";
    	}
